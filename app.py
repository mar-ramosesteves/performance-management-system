...
def calculate_nine_box_position(performance, potential):
    ...
    def rating_to_9box(rating):
        """Converte rating (1-5) para 9-box (1-9) usando a tabela de correlação real"""
        # Tabela de correlação real baseada na sua imagem
        correlation_table = {
        ...
        # Arredondar para o valor mais próximo na tabela
        rounded_rating = round(rating, 1)
        
        # Se o valor exato não existir, interpolar
        if rounded_rating in correlation_table:
            ...
            upper = lower + 1
            if upper <= 5:
                lower_val = correlation_table.get(lower, 0)
                upper_val = correlation_table.get(upper, 0)
                result = lower_val + (upper_val - lower_val) * (rating - lower)
            else:
                result = correlation_table[5.0]
